...
def main():
    ...
    # Load the datasets (here the change of dataset to load it )
    ...
    # Array to store the validation losses
    validation_losses = []
    ...
    for epoch in range(start_epoch, ending_epoch):
        ...
        if run_validation or run_nas_validation:
            ...
            with collectors_context(activations_collectors["valid"]) as collectors:
                if not args.dr:
                    top1, top5, vloss, mAP, validation_loss, conf_matrix = validate(val_loader, model, criterion, [pylogger],
                                                      args, epoch, tflogger)
                    # Store the validation loss in the array
...
# Import libraries & define the GUI backend
import matplotlib
matplotlib.use('TkAgg')  # Tkinter backend for interactive use
import matplotlib.pyplot as plt

# Function to graph the "Loss per Epoch"
...
def test(test_loader, model, criterion, loggers, activations_collectors, args):
    ...
    with collectors_context(activations_collectors["test"]) as collectors:
        top1, top5, vloss, mAP, validation_loss, conf_matrix = _validate(test_loader, model, criterion, loggers, args)
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1, tflogger=None):
    ...
    if not args.earlyexit_thresholds:
        ...
        if not args.regression:
            ...
            return classerr.value(), .0, losses[OBJECTIVE_LOSS_KEY].mean, 0, losses[OBJECTIVE_LOSS_KEY].mean, confusion.value()
        ...
        if not args.regression:
            return classerr.value(1), classerr.value(min(args.num_classes, 5)), \
                losses[OBJECTIVE_LOSS_KEY].mean, 0, losses[OBJECTIVE_LOSS_KEY].mean, confusion.value()

    # else:
    total_top1, total_top5, losses_exits_stats = earlyexit_validate_stats(args)
    return total_top1, total_top5, losses_exits_stats[args.num_exits-1], 0, losses[OBJECTIVE_LOSS_KEY].mean, confusion.value()
